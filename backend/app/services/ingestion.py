...
class IngestionManager:
    ...
    @staticmethod
    async def ingest_company_data(ticker: str, filing_type: str = "10-K") -> dict:
        """
        ...
        try:
            ...
            company_info, financials, prices = await asyncio.gather(
            ...
            if company_info and company_info.get("website"):
                scraped_logo = image_scraper_service.get_logo_for_ticker(
                    ticker, company_info.get("website")
                )
                if scraped_logo:
                    with Session(engine) as session:
                        db_company = session.exec(
