...
class ModelingAgent:
    ...
    async def _fetch_rag_context(self, ticker: str) -> dict:
        ...
        try:
            results = vector_store_service.query(
                query_texts=["fiscal outlook guidance growth margins risks opportunities capital allocation"],
                n_results=10,
                where={"ticker": ticker}
            )
            ...
            return {
                "text": "\n---\n".join(context_parts) if context_parts else "No specific context found in filings.",
                "sources": list(set([m.get("source_url") for m in metas if m.get("source_url")]))
            }
    ...
    async def _get_ai_assumptions(self, company: Company, hist: dict, context_data: dict) -> dict:
        ...
        """
        prompt = f"""
        Analyze the following financial data and qualitative context for {company.name} ({company.ticker}).
        ...
        - reasoning (string): Concise explanation. 
          IMPORTANT: You MUST cite the source URLs provided in the context for any specific claims or guidance mentioned. 
          Format citations as markdown links: [Source Name or Date](URL).
        - thoughts (string): A step-by-step summary of how you arrived at these numbers (e.g., "Analyzed revenue CAGR of 15% and noted pressure on margins due to X...").
        ...
        try:
            ...
            if settings.PRIMARY_LLM_PROVIDER == "google" and settings.GOOGLE_API_KEY:
                ...
                # Gemini often works better with a direct prompt than system/human split for structured JSON
            ...
            source_url = company.latest_filing_url or "https://www.sec.gov/edgar/searchedgar/companysearch"
            source_link = f" ([Latest SEC Filing]({source_url}))"
            return {
                "BASE": {
                    "growth_mod": 0.0, 
                    "margin_mod": 0.0, 
                    "reasoning": f"Standard historical extrapolation based on past performance.{source_link}",
                ...
                "BULL": {
                    "growth_mod": 0.02, 
                    "margin_mod": 0.01, 
                    "reasoning": f"Generic upside scenario assuming improved market conditions.{source_link}",
                    "thoughts": "Upside projection based on potential sector tailwinds and operational efficiency.",
                    "sources": [source_url],
                    "is_fallback": True
                },
                "BEAR": {
                    "growth_mod": -0.03, 
                    "margin_mod": -0.02, 
                    "reasoning": f"Generic downside scenario based on sector risk factors.{source_link}",
                    "thoughts": "Downside risks primarily driven by macroeconomic volatility and margin compression.",
                    "sources": [source_url],
                    "is_fallback": True
                }
            }
    ...
    def _create_scenario(self, company: Company, hist: dict, s_type: str, g_mod: float, m_mod: float, reasoning: str = "", thoughts: str = "", sources: list[str] = [], is_fallback: bool = False) -> ForecastScenario:
        res = ForecastScenario(
    ...
    def _calculate_dcf(self, scenario: ForecastScenario, company: Company):
        """
        ...
        denom = wacc - g
        if denom <= 0:
            logger.warning("WACC <= Terminal Growth, adjusting spread for calculation", wacc=wacc, g=g)
