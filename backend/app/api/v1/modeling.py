...
async def get_forecast(ticker: str, session: Session = Depends(get_session)):
    """
    ...
    try:
        ...
        if not company or not company.is_ingested:
            logger.info("Company not yet fully ingested, returning processing status", ticker=ticker)
            return {
                "status": "processing",
                "ticker": ticker,
                "scenarios": []
        ...
        scenarios = session.exec(
            ...
            .order_by(ForecastScenario.scenario_type.asc())
        ).all()
        ...
        raise HTTPException(status_code=500, detail="Internal server error during modeling.")

@router.get("/{ticker}/sensitivity")
async def get_sensitivity(ticker: str, session: Session = Depends(get_session)):
    """
    ...
    try:
        ...
        base_scenario = session.exec(
            select(ForecastScenario)
            .where(ForecastScenario.company_id == company.id, ForecastScenario.scenario_type == "BASE")
        ).first()
        ...
        g_range = [base_scenario.terminal_growth - 0.005, base_scenario.terminal_growth, base_scenario.terminal_growth + 0.005]
...
def _recalc_intrinsic_only(scenario, company, wacc, g):
